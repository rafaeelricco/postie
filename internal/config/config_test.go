...
func TestFixtures(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			path := filepath.Join("..", "..", "tests", "fixtures", "config", tc.file)
...
func TestMultiDestinationConfigLoads(t *testing.T) {
	...
	path := filepath.Join("..", "..", "tests", "fixtures", "config", "multi-destination.yaml")
...
func TestLoadEngineRejectsUnknownKind(t *testing.T) {
	config := baseEngineConfig + `destinations:
  Accounts_Projection:
    kind: bogus
`
	_, err := LoadEngine(writeEngineConfig(t, config))
	if err == nil {
...
control:
  database_url: postgres://control
`
	if _, err := LoadEngine(writeEngineConfig(t, config)); err == nil {
...
func TestSourcePolicyPublicationLoads(t *testing.T) {
	config := baseEngineConfig + `sources:
  events:
    publication: events_pub
`
	e, err := LoadEngine(writeEngineConfig(t, config))
	if err != nil {
		t.Fatalf("expected config to load, got error: %v", err)
	}
