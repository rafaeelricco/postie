...
func TestLogWindowAndCursors(t *testing.T) {
	...
	log := activity.New(&output)
	empty, err := log.Read("")
	if err != nil || empty.Entries == nil || len(empty.Entries) != 0 || empty.Reset {
		t.Fatalf("empty: %+v %v", empty, err)
	}
	for i := 0; i < 1005; i++ {
		log.Add(activity.Entry{Message: "delivered", EventID: fmt.Sprint(i)})
	}
	page, err := log.Read("")
	...
	log.Add(activity.Entry{Level: "warn", Message: "retry", Attempt: 1})
	next, err = log.Read(page.NextCursor)
	...
	restarted, err := activity.New(nil).Read(page.NextCursor)
...
func TestLogConcurrentReadersOwnTheirSnapshots(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 100; j++ {
				log.Add(activity.Entry{Message: "attempt"})
				page, _ := log.Read("")
