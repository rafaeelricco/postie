...
func TestClientAppliesConfigurationAndReadsRunningStatus(t *testing.T) {
	for _, code := range []int{http.StatusOK, http.StatusCreated} {
		t.Run(fmt.Sprint(code), func(t *testing.T) {
			...
			client := debezium.Client{URL: server.URL + "/", Connections: map[string]debezium.Connection{source.ID: testConnection()}}
...
func TestClientRejectsMalformedConnectorEndpoint(t *testing.T) {
	client := debezium.Client{URL: "http://invalid\nendpoint", Connections: map[string]debezium.Connection{}}
	source := testSource()
