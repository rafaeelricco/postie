...
func testConnection() debezium.Connection {
	return debezium.Connection{Host: "db.internal", Port: 5432, Username: "engine", Password: "s3cr3t-password", Database: "app"}
}
...
func TestConnectorConfigKeyIsPartitioningColumn(t *testing.T) {
	...
	cfg := debezium.ConnectorConfig(src, testConnection(), testIdentity(src), n, provision.PublicationManaged)
...
func TestConnectorConfigSnapshotOrdersBySerial(t *testing.T) {
	...
	n := provision.NamesFor("postie", "production", src, 1)
	id := testIdentity(src)
	cfg := debezium.ConnectorConfig(src, testConnection(), id, n, provision.PublicationManaged)
...
func TestEnsureConnectorNeverLeaksPassword(t *testing.T) {
	...
	n := provision.NamesFor("postie", "production", src, 1)
	cfg := debezium.ConnectorConfig(src, connection, testIdentity(src), n, provision.PublicationManaged)

	err := debezium.EnsureConnector(context.Background(), server.Client(), server.URL, n, cfg)
	if err == nil {
...
func TestConnectorStatusMissing(t *testing.T) {
	...
	_, err := debezium.ConnectorStatus(context.Background(), server.Client(), server.URL, "missing-connector")
...
func TestConnectorConfigPublicationMode(t *testing.T) {
	...
	for _, c := range cases {
		cfg := debezium.ConnectorConfig(src, testConnection(), id, n, c.mode)
