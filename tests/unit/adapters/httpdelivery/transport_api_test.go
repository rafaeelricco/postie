...
func TestLeaseTransportAppliesLeaseDeadline(t *testing.T) {
	...
	_, err = (httpdelivery.LeaseTransport{Gate: transportGate{allowed: true, deadline: wantDeadline}, Base: base}).RoundTrip(req)
...
func TestLeaseTransportCancelsInFlightBodyWhenClosed(t *testing.T) {
	...
	response, err := (httpdelivery.LeaseTransport{Gate: transportGate{allowed: true, deadline: time.Now().Add(time.Minute)}, Base: base}).RoundTrip(req)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLeaseTransportPropagatesRequestCancellation(t *testing.T) {
	...
	go func() {
		_, err := (httpdelivery.LeaseTransport{Gate: transportGate{allowed: true, deadline: time.Now().Add(time.Minute)}, Base: base}).RoundTrip(req)
		done <- err
	}()
	cancel()
