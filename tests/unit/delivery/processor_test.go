...
func (s *fakeSender) Send(_ context.Context, _ stream.Record, body []byte) (delivery.Outcome, error) {
	s.calls++
...
func TestProcessorFiltersAndMarshalsOnce(t *testing.T) {
	sender := &fakeSender{outcome: delivery.Delivered}
	p := delivery.NewProcessor(delivery.Destination{ID: "d", Description: "destination", Filter: &protocol.Filter{Column: "kind", Values: []string{"ok"}}}, sender)
	...
	if err != nil || outcome != delivery.Filtered || sender.calls != 0 {
	...
	if err != nil || outcome != delivery.Delivered || sender.calls != 1 {
		t.Fatalf("got %v %v calls=%d", outcome, err, sender.calls)
	}
...
func TestProcessorRetriesAndCallsPerAttempt(t *testing.T) {
	sender := &fakeSender{outcome: delivery.Delivered, errs: []error{errors.New("temporary"), nil}}
	p := delivery.NewProcessor(delivery.Destination{ID: "d"}, sender)
	...
	if err != nil || outcome != delivery.Delivered || sender.calls != 2 {
		t.Fatalf("got %v %v calls=%d", outcome, err, sender.calls)
	}
