...
func TestNamesForIsDeterministicAndValid(t *testing.T) {
	...
	for _, c := range cases {
		n := provision.NamesFor(c.namespace, c.environment, c.src, c.generation)
		results[c.name] = n
		...
		if again := provision.NamesFor(c.namespace, c.environment, c.src, c.generation); again != n {
...
func TestIdentityFromRejections(t *testing.T) {
	...
	validFacts := func() provision.TableFacts {
		return provision.TableFacts{Exists: true, Columns: map[string]provision.ColumnFacts{
	...
	cases := []struct {
		name  string
		facts func() provision.TableFacts
	}{
		{"missing-table", func() provision.TableFacts { f := validFacts(); f.Exists = false; return f }},
		{"missing-column", func() provision.TableFacts { f := validFacts(); delete(f.Columns, "payload"); return f }},
		{"nullable-serial", func() provision.TableFacts {
			f := validFacts()
			f.Columns["id"] = provision.ColumnFacts{Type: stream.PGInt8}
			return f
		}},
		{"text-serial", func() provision.TableFacts {
			f := validFacts()
			f.Columns["id"] = provision.ColumnFacts{Type: stream.PGText, NotNull: true}
			return f
		}},
		{"no-unique-index-on-serial", func() provision.TableFacts { f := validFacts(); f.SerialUniqueIndex = false; return f }},
		{"nullable-partitioning", func() provision.TableFacts {
			f := validFacts()
			f.Columns["correlation_id"] = provision.ColumnFacts{Type: stream.PGText}
			return f
		}},
		{"jsonb", func() provision.TableFacts {
			f := validFacts()
			f.Columns["payload"] = provision.ColumnFacts{Type: stream.PGType("jsonb"), NotNull: true}
			return f
		}},
		{"uuid", func() provision.TableFacts {
			f := validFacts()
			f.Columns["payload"] = provision.ColumnFacts{Type: stream.PGType("uuid"), NotNull: true}
			return f
		}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, err := provision.IdentityFrom(src, c.facts(), 5)
			if err == nil {
				t.Fatal("identityFrom succeeded, want an error")
			}
			var contractErr *provision.ContractError
	...
	got, err := provision.IdentityFrom(src, validFacts(), 5)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIdentityFromAcceptsNullableJSON(t *testing.T) {
	src := testSource()
	facts := provision.TableFacts{Exists: true, Columns: map[string]provision.ColumnFacts{
		"id":             {Type: stream.PGInt8, NotNull: true},
		"correlation_id": {Type: stream.PGText, NotNull: true},
		"payload":        {Type: stream.PGJSON},
	}, SerialUniqueIndex: true}

	got, err := provision.IdentityFrom(src, facts, 3)
	if err != nil {
